...
image_labeld_indexs = [] # Annotated image index
image_not_labeld_indexs = list(range(len(test_examples))) # Unmarked image index
...
def load_next_image(annotated_images_count,input_img,filename_display):
    ...
    global image_labeld_indexs
    global image_not_labeld_indexs
    if annotated_images_count + 1 >= len(test_examples):
        # If the image has already been marked, prompt the user
        next_index = annotated_images_count
        gr.Info("All the images are labeled！")
    else:
        next_index = image_not_labeld_indexs[0]
        # return (annotated_images_count,"**Progress:** 0/0","**Current Image File Name:** None",None,None,None,None,None,None,None,None,None,None,None,None)
    ...
    image_labeld_indexs.append(current_image_id)
    try:
        image_not_labeld_indexs.remove(current_image_id)
    except:
        pass
    choices = [(image_id2name[i],i) for i in image_not_labeld_indexs]
    choices.extend([(f"【Labeled:】{image_id2name[i]}",i) for i in image_labeld_indexs])
...
5. Update file name display
    ...
    current_image_name = filename_display.replace("**Current Image File Name:** ","")
    current_image_id = image_name2id[current_image_name]
    # print(current_image_name,current_image_id)
    # print(image_id,image_id2name[image_id])
    global image_labeld_indexs
    global image_not_labeld_indexs
    if image_id in image_labeld_indexs: # If the image has already been annotated, the number of annotated images will not be increased
        annotated_images_count = len(image_labeld_indexs)
        gr.Info("This image has been labeled!")
    else:
        # print(image_not_labeld_indexs,current_image_id)
        image_labeld_indexs.append(current_image_id)
        try:
            image_not_labeld_indexs.remove(current_image_id)
        except:
            pass
        annotated_images_count = len(image_labeld_indexs)
    current_image_name = image_id2name[image_id]
    current_image_filename = test_examples[image_id]
    next_image_path = test_examples[image_id]
    choices = [(image_id2name[i],i) for i in image_not_labeld_indexs]
    choices.extend([(f"【Labeled:】{image_id2name[i]}",i) for i in image_labeld_indexs])
    click_img, scribble_img, output_img, click_coords, click_labels, bbox_coords, \
        seperate_scribble_masks, last_scribble_mask, best_mask, low_res_mask, img_features = clear_all_history(input_img)
    filename_display_text = f"**Current Image File Name:** {current_image_name}"
    progress_display_text = f"**Progress:** {annotated_images_count}/{len(test_examples)}"
    image_list = gr.Dropdown(
                choices = choices,
                value = image_id,
