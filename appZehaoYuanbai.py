...
# Get the image file name without the prefix path
# test_image_names = [example.split("/")[-1] for example in test_examples]
test_image_names = [os.path.basename(example) for example in test_examples]
# To display a dropdown list and distinguish between labeled and unlabeled items
image_file_indexs = list(range(len(test_examples))) # All image indexes
image_labeled_indexs = [] # Annotated image index
image_not_labeled_indexs = list(range(len(test_examples))) # Unmarked image index
image_id2name = {i: name for i, name in enumerate(test_image_names)} # Mapping from image index to image name
image_name2id = {name: i for i, name in enumerate(test_image_names)} # Mapping from image name to image index
default_choices = [(test_image_names[i], i) for i in image_file_indexs] # Default Options
...
def refresh_predictions(predictor, input_img, output_img, click_coords, click_labels, bbox_coords, brush_label,
    ...
    # If the user uploads a volume (3D/video) and the current slice has a value, it will be used as the predicted input image
...
# =============================================================================
# Processing functions for 3D images and videos
# =============================================================================

def load_volume_slice(file_path, slice_index):
    """
    Load 3D image (such as NIfTI format) and return the specified slice (assuming the slice is in the third dimension)
    """
    volume = nib.load(file_path).get_fdata()  # Get numpy array
    num_slices = volume.shape[2]
    if slice_index < 0 or slice_index >= num_slices:
        raise ValueError(f"Slice index {slice_index} Out of range 0 - {num_slices - 1}")
...
def load_npy_slice(file_path, slice_index):
    """
    Load the 3D array in the. npy file and return the specified slice of the third dimension
Assuming the shape of the array is (H, W, D)
    """
    volume = np.load(file_path)
    if volume.ndim != 3:
        raise ValueError(f"Expecting a 3D array (H, W, D), but obtaining a shape: {volume.shape}")
    num_slices = volume.shape[2]
    if slice_index < 0 or slice_index >= num_slices:
        raise ValueError(f"Slice index {slice_index} Out of range 0 - {num_slices - 1}")
    slice_img = volume[:, :, slice_index]
...
def get_temp_file(file_obj):
    # If file_obj is a dictionary, try reading file_obj ["data"]
    if isinstance(file_obj, dict) and "data" in file_obj:
        ...
        return temp.name
    # If you directly pass in the file path or file object
...
def load_video_frame(file_path, frame_index):
    ...
    print(f"Total number of frames in the video: {total_frames}, current index: {frame_index}")
    ...
    cap.release()
    if not ret:
        raise IOError(f"Unable to read the {frame_index} frame")
...
def update_current_slice(volume_file, slider_value):
    if volume_file is None:
        return None
    # If volume_file is a dictionary, try extracting the file path or writing to a temporary file
    ...
    if file_ext in ['.nii', '.nii.gz']:
        try:
            ...
            print("Error loading 3D image：", e)
            return None
        ...
        try:
            ...
            return None
    elif file_ext == '.npy':  # <-- Processing. npy
        try:
            img = load_npy_slice(file_path, idx)
        except Exception as e:
            print("Error loading. npy:", e)
            return None
    else:
...
def load_next_image(annotated_images_count,input_img,filename_display):
    '''The operation to start when clicking on the Next Image
    '''
    # Variables that need to be changed
    global image_labeled_indexs
    global image_not_labeled_indexs
    # The currently annotated image
    current_image_name = filename_display.replace("**Current Image File Name:** ","")
    current_image_id = image_name2id[current_image_name]
    
    # Remove the current image from the unlabeled list and add it to the annotated list
    if current_image_id not in image_labeled_indexs:
        image_labeled_indexs.append(current_image_id)
        try:
            image_not_labeled_indexs.remove(current_image_id)
        except:
            pass
    # Determine whether all images have been annotated
    if annotated_images_count + 1 >= len(test_examples):
        # If the image has already been marked, prompt the user and keep the current image unchanged
        next_index = current_image_id
        annotated_images_count = len(test_examples)
        gr.Info("All the images are labeled！")
    else:
        # If the annotation is not completed yet, select an image from the list of unlabeled images
        next_index = image_not_labeled_indexs[0]
    
    
    # Update the next image to be annotated
    try:
        next_image_path = test_examples[next_index]
    except:
        next_image_path = test_examples[current_image_id]
    annotated_images_count = len(image_labeled_indexs)
    # Update file name display
    file_name = os.path.basename(next_image_path)
    filename_display_text = f"**Current Image File Name:** {file_name}"
    
    # Update progress bar
    progress_display_text = f"**Progress:** {annotated_images_count}/{len(test_examples)}"
    
    # Update the dropdown list and label the annotated images as labeled
    choices = [(image_id2name[i],i) for i in image_not_labeled_indexs]
    choices.extend([(f"【Labeled:】{image_id2name[i]}",i) for i in image_labeled_indexs])
    image_list = gr.Dropdown(
                choices = choices,
                value = image_name2id[file_name],
                interactive = True,
                label = "Image List"
            )
    # Clear recording, not in the field of images, unable to understand the specific operation
    click_img, scribble_img, output_img, click_coords, click_labels, bbox_coords, \
        seperate_scribble_masks, last_scribble_mask, best_mask, low_res_mask, img_features = clear_all_history(input_img)
    
    return (image_list,annotated_images_count,progress_display_text,filename_display_text,
            next_image_path,next_image_path,next_image_path,output_img,
            click_coords, click_labels, bbox_coords,seperate_scribble_masks, 
            last_scribble_mask, best_mask, low_res_mask, img_features)

def select_image(image_list,input_img,filename_display):
    '''The operation that starts when selecting an image from the drop-down list
1. Update the number of annotated images
2. Update the index list of annotated and unlabeled images
3. Update the current image
4. Update progress bar
5. Update file name display
    '''
    # Variables that need to be changed
    global image_labeled_indexs
    global image_not_labeled_indexs
    ...
    image_id = image_list
    # Retrieve the file name and ID of the currently annotated image
    current_image_name = filename_display.replace("**Current Image File Name:** ","")
    current_image_id = image_name2id[current_image_name]
    # Determine whether the selected image has been annotated
    if image_id in image_labeled_indexs: 
        # If the image has already been annotated, it will prompt that it has been annotated
        gr.Info("This image has been labeled!")
    # Process the current image, add it to the annotated list, and remove it from the unlabeled list
    if current_image_id not in image_labeled_indexs:
        image_labeled_indexs.append(current_image_id)
    try:
        image_not_labeled_indexs.remove(current_image_id)
    ...
    next_image_path = test_examples[image_id]
...
with gr.Blocks(theme=gr.themes.Default(text_size=gr.themes.sizes.text_lg)) as demo:
    ...
    # Add a new state variable to record the number of annotated images
    annotated_images_count = gr.State(0)
    ...
    # Add additional content that needs to be displayed
    with gr.Row():
        # Add dropdown menu
        image_list = gr.Dropdown(
        ...
        with gr.Row():
            with gr.Column(scale=1):
                # Add a component to display task progress
                progress_display = gr.Markdown(f"**Progress**: 0 / {len(test_examples)}")
                # Add a component to display the file name of the current annotated image
                filename_display = gr.Markdown(f"**Current Image File Name:** {os.path.basename(default_example)}")
            
            # Add button to complete the current annotation and proceed to the annotation of the next image
            next_image_button = gr.Button("Next Image", variant="primary")
    ...
    next_image_button.click(
        fn=load_next_image,
        inputs=[annotated_images_count,input_img,filename_display],
        outputs = [image_list,annotated_images_count,progress_display,filename_display,
                input_img, click_img, scribble_img, 
                output_img, click_coords, click_labels, bbox_coords, 
                seperate_scribble_masks, last_scribble_mask, 
                best_mask, low_res_mask, img_features
        ]
    ...
    # List selection triggers events
    image_list.select(
        fn = select_image,
        inputs = [image_list,input_img,filename_display],
        outputs = [image_list,annotated_images_count,progress_display,filename_display,
                input_img, click_img, scribble_img, 
                output_img, click_coords, click_labels, bbox_coords, 
                seperate_scribble_masks, last_scribble_mask, 
    ...
    # -------------------------------------------
    # Clear the current prediction mask
    ...
    # -------------------------------------------
    # Update current slice/frame display when slider changes
    frame_slider.change(
    ...
    # Modify the callback of the Refresh Prediction button by additionally passing volume_input and Current_stlice
    submit_button.click(
