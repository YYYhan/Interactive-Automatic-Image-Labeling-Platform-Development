...
model_dict = {
    ...
    'ScribblePrompt-SAM': 'ScribblePrompt_sam_v1_vit_b_res128.pt'
}
...
def update_image_selector(image_files):
    """Update the dropdown choices based on uploaded files."""
    if image_files is None or len(image_files) == 0:
        return gr.update(choices=[])
    else:
        # Use tuples of (label, value) where label is the file name and value is the file path
        choices = [(os.path.basename(f.name), f.name) for f in image_files]
        return gr.update(choices=choices)

def load_selected_image(selected_path):
    """Load the selected image as a grayscale numpy array."""
    if selected_path is None:
        return None
    else:
        # Load as grayscale since image_mode="L" and app expects 2D images
...
def get_scribbles(seperate_scribble_masks, last_scribble_mask, scribble_img):
    ...
    Record scribbles
    """
    assert isinstance(seperate_scribble_masks,
                      np.ndarray), "seperate_scribble_masks must be numpy array, got type: " + str(
        type(seperate_scribble_masks))
...
def refresh_predictions(predictor, input_img, output_img, click_coords, click_labels, bbox_coords, brush_label,
    ...
    mask_img = 255 * (mask_to_viz[..., None].repeat(axis=2, repeats=3) > 0.5) if binary_checkbox else mask_to_viz[
...
with gr.Blocks(theme=gr.themes.Default(text_size=gr.themes.sizes.text_lg)) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            with gr.Accordion("Troubleshooting tips", open=False):
                gr.Markdown(
                    "<span style='color:orange'>If you encounter an <span style='color:orange'>error</span> try clicking 'Clear All Inputs'.")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            with gr.Tab("Input Image"):
                image_files = gr.File(file_count="multiple", label="Upload Images", type="filepath")
                image_selector = gr.Dropdown(label="Select Image", choices=[])
                input_img = gr.Image(
                    ...
                    container=True,
                    height=display_height,
                    value=default_example  # Keep default example initially
                )
                gr.Markdown(
    ...
    # When image_files changes, update the dropdown choices
    image_files.change(
        fn=update_image_selector,
        inputs=image_files,
        outputs=image_selector
    )

    # When image_selector changes, load the selected image and update input_img
